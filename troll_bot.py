...
class MyStreamListener(tweepy.StreamListener):
    ...
    def on_status(self, current_tweet):
        ...
        if from_creator(current_tweet):
            ...
            # have a blanket best subject be 'SMOKING' in the case that the subject returns empty due to the filter
            best_subject = 'SMOKING'
            max_len_subject = -1
            ...
            message = f'Troll Bot tweeted: {tweet_to_send}'
            print(message)
            send_text(message)
            return message
        else:
    ...
    def on_error(self, status):
        ...
        message = f'Error detected on Troll Bot: {status}'
        print(message)
        send_text(message)
        return message
